...
type Config struct {
	...
	ExhaustiveAggregation bool // raise aggregation-question recall to topK=500 and sweep the full result set into context

	// H12: enumerate-first generation prompt
	EnumerateFirst bool // prepend enumerate-first instruction for aggregation questions (default off)
...
func dispatch(args []string, stdout, stderr io.Writer) int {
	...
	// H8: exhaustive aggregation recall
	fs.BoolVar(&cfg.ExhaustiveAggregation, "exhaustive-aggregation", false, "H8: raise aggregation-question recall to topK=500 and sweep the full result set into context (default off)")
	// H12: enumerate-first generation prompt
	fs.BoolVar(&cfg.EnumerateFirst, "enumerate-first", false, "H12: prepend an enumerate-first instruction for aggregation questions (default off)")
