...
func run() error {
	...
	embedRecallTimeoutMS := fs.Int("embed-recall-timeout-ms", envInt("ENGRAM_EMBED_RECALL_TIMEOUT_MS", 1500), "Embed call budget during recall before degrading to BM25+recency (ms; 0 = no embed deadline, parent context governs — caller must supply a bounded context)")
