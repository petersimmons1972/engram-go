...
- **Hybrid** — external Ollama/Olla router, started by `make up` via `docker-compose.yml`. Requires `ENGRAM_ROUTER_URL` in `.env` (legacy `LITELLM_URL` fallback accepted, retained for backward compatibility; see issue #634). PostgreSQL defaults to the bundled `postgres` service; set `POSTGRES_HOST` / `POSTGRES_PORT` only when you are pointing Engram at an external database.
...
# Hybrid `.env`: set `ENGRAM_ROUTER_URL` (or legacy `LITELLM_URL`) for your
# external router. `POSTGRES_HOST` / `POSTGRES_PORT` default to `postgres:5432`
# and only need overrides for an external PostgreSQL server.
make init
make up
make setup
```
...
| `POSTGRES_HOST` | PostgreSQL host for hybrid compose | `postgres` |
| `POSTGRES_PORT` | PostgreSQL port for hybrid compose | `5432` |
