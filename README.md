...
- **Hybrid** — external Ollama/Olla router, started by `make up` via `docker-compose.yml`. Requires `ENGRAM_ROUTER_URL` in `.env` (legacy `LITELLM_URL` fallback accepted, retained for backward compatibility; see issue #634).
