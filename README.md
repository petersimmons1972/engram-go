...
- **Hybrid** — Route advanced models (Qwen, Claude) through an external olla load balancer. Default `docker-compose.yml`. Requires `LITELLM_URL` in `.env` (variable name retained for backward compat; the LiteLLM proxy container was retired 2026-05-05 — see `docker-compose.yml` header comment and issue #634).
...
```bash
# Required external Docker networks (`litellm_default`, `ai-network`) are
# created automatically by `make up` via the `ensure-networks` target. They
# are declared external so they can be shared with a co-located olla/LiteLLM
...
**For local-only setup** (Ollama-only, no external LLM proxy):
...
| `LITELLM_URL` | External OpenAI-compatible endpoint (olla load balancer; name retained from retired LiteLLM container) | `http://olla:40114/olla/openai` |
...
| Local-first vs hybrid (olla), personal infra notes | [Deployment Notes](docs/deployment-notes.md) |
...
| [MCP Tool Reference](docs/tools.md) | MCP tool reference with usage examples |
