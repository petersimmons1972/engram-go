...
// TestSetupTokenRequiresBearerAuth tests that /setup-token now requires
// a valid bearer token and rejects unauthenticated requests (#540 B1).
//
// Before fix: unauthenticated local IP requests succeed (FAIL).
// After fix: only valid bearer tokens are accepted (PASS).
func TestSetupTokenRequiresBearerAuth(t *testing.T) {
	s := &Server{
		trustProxy: false,
	}
...
// TestSetupTokenRejectsXRealIPSpoofingWithoutAuth tests that spoofing
// X-Real-IP without valid bearer auth is rejected (#551 regression test).
func TestSetupTokenRejectsXRealIPSpoofingWithoutAuth(t *testing.T) {
	s := &Server{
