...
func TestFormatAtomsAsContext_EntityWithEmptyPolarity(t *testing.T) {
	...
	assert.NotContains(t, result, "[LIKES:", "empty polarity must not invent LIKES tag")
	assert.NotContains(t, result, "[AVOIDS:", "empty polarity must not invent AVOIDS tag")
}

// TestFormatAtomsAsContext_UnknownPolarityNoTag is adversarial: a garbage
// polarity value must not emit a tag that confuses the generator.
func TestFormatAtomsAsContext_UnknownPolarityNoTag(t *testing.T) {
	atoms := []atom.Atom{
		makeTestAtomWithEntity("a1", "maybe", "matcha", "food", "The user likes matcha."),
	}
	result := search.FormatAtomsAsContext(atoms)
	assert.NotContains(t, result, "[LIKES:")
	assert.NotContains(t, result, "[AVOIDS:")
	assert.NotContains(t, result, "maybe")
	assert.Contains(t, result, "The user likes matcha.")
}

// TestFormatAtomsAsContext_PolarityWithoutEntity is adversarial: polarity alone
// must not invent an entity tag from Value or Statement.
func TestFormatAtomsAsContext_PolarityWithoutEntity(t *testing.T) {
	a := makeTestAtom("a1", atom.TypePreference, "The user prefers tea.", 0.9)
	a.Polarity = "like"
	a.Value = "tea"
	result := search.FormatAtomsAsContext([]atom.Atom{a})
	assert.NotContains(t, result, "[LIKES:", "polarity without entity must not invent LIKES tag")
}
