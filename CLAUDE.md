...
- **codex-guard for shell ops:** Agent briefs that run git/gh/kubectl/helm must prefix those calls with `codex-guard` (e.g. `codex-guard git push origin main`). Never brief an agent to call `git *` or `gh *` directly — the `Bash(git *)` allow is gone; only `Bash(codex-guard *)` is permitted. Use `mode: "bypassPermissions"` on every agent dispatch to prevent permission-prompt chains (alert fatigue is a worse security risk than the ops being prompted about).
  **codex-guard runs only on the LOCAL host.** For remote git/gh/kubectl/helm over ssh, guard the ssh invocation locally: `codex-guard ssh <host> '<cmd>'` (the guard inspects the inner command). **Never** invoke codex-guard inside the remote shell — `ssh host 'codex-guard …'` is WRONG (codex-guard does not exist on remote hosts and the call fails).
  - CORRECT: `codex-guard ssh codex.petersimmons.com 'git push origin main'`
  - WRONG:   `ssh codex.petersimmons.com 'codex-guard git push origin main'`  ← codex-guard absent on remote → `command not found`
...
| 9 | **codex-guard** | All git/gh/kubectl/helm calls in agent briefs must be prefixed with `codex-guard` (e.g. `codex-guard git commit -m "…"`). `Bash(git *)` is not in the allow list — only `Bash(codex-guard *)` is. Every agent dispatch uses `mode: "bypassPermissions"` to prevent permission-prompt alert fatigue. **LOCAL ONLY:** codex-guard runs only on the local host. For remote ops over ssh, guard the ssh call locally — `codex-guard ssh <host> '<cmd>'` — and never invoke codex-guard inside the remote shell (`ssh host 'codex-guard …'` fails: binary absent on remote). CORRECT: `codex-guard ssh host 'git push origin main'` · WRONG: `ssh host 'codex-guard git push origin main'`. |
...
**codex-guard enforcement hierarchy** → `~/docs/codex-guard-enforcement-hierarchy.md` — layered model (deny-list → git hooks → SSH ForceCommand → local audit log); replaces the fiction that "prefix codex-guard = primary enforcement."
...
**Night Protocols** (autonomous overnight operation) → `~/docs/night-protocols.md` — reusable authority model, bus-only comms, build-proof gate, deferred-supervised batch pattern.
