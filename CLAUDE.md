...
- **`longmemeval atom-build` embedding safety.** Always pass `--embed-url` explicitly; it is required and is not inferred from `--llm-url`. Pass the embedding base with or without one trailing `/v1` (the client normalizes it before appending `/v1/embeddings`). A run that processes at least one session but stores zero atoms exits nonzero; an empty work list still succeeds. A session that extracted atoms but persisted none is checkpointed `status:"error"`, not `"done"`, so a re-run reprocesses it — a `"done"` there would put it in the skip set and make the next run exit 0 with no work while the atoms are still missing. Atom storage remains blind-insert without semantic deduplication, so rerunning against the same project can duplicate atoms. `valid_from` remains limited to event/status-change atoms, using the extracted event date or the source session date; other atom types carry `observed_at` only.
