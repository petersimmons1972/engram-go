...
- **Destructive shell ops are blocked automatically** by the codex-guard PreToolUse hook — agents do NOT need to prefix commands with `codex-guard`. The hook runs on every Bash call via `~/.claude/hooks/codex-guard-bash-guard.sh`; the settings.json deny-list provides a mechanical backstop. Use `mode: "bypassPermissions"` on every agent dispatch to prevent permission-prompt chains (alert fatigue is a worse security risk than the ops being prompted about).
...
| 9 | **destructive-op gate (automatic)** | Destructive shell commands are blocked automatically by the codex-guard PreToolUse hook + the settings.json deny-list — agents do NOT prefix commands with `codex-guard`. The prefix mandate is RETIRED; briefing it wastes tokens and was bypassed-when-forgotten anyway. The hook (`~/.claude/hooks/codex-guard-bash-guard.sh`) fires on every Bash call, fails-open if codex-guard is unavailable, and logs misses to `~/.claude/codex-guard-hook.log`. |
...
**codex-guard enforcement hierarchy** → `~/docs/codex-guard-enforcement-hierarchy.md` — layered model: Layer 1 = PreToolUse hook (automatic, primary gate); Layer 2 = settings.json deny-list (backstop); Layers 3–4 = server-side git hooks + SSH ForceCommand (planned). The prefix mandate is RETIRED.
