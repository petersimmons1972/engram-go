...
**Don't use when:**
- Failures share a confirmed root cause — this is confirmed by Phase 1 investigation, not by guessing at the phrase "might fix others." If you have not yet done Phase 1, you do not know whether failures are related; run Phase 1 first, then decide.
- Need to understand full system state before acting (one agent investigates first, parallel dispatch comes after)
- Agents would interfere with each other (editing same files, holding same locks, using same external resources)
...
**Exploratory debugging:** You have one failure and don't yet know whether it represents one bug or multiple. Run Phase 1 investigation to determine N. Parallelize only after Phase 1 confirms N independent root causes.
