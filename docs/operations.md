...
**`GET /setup-token`** — returns the current bearer token, SSE endpoint URL, and server name as JSON. Requires the header `"Authorization: Bearer <ENGRAM_API_KEY>"`. Localhost-only (accepts loopback `127.0.0.1` / `::1` and RFC1918 Docker bridge addresses; rejects all others). Used by `make setup` to configure MCP clients without manual copy-paste.
...
All other routes (`/sse`, memory operation POSTs) require the header `"Authorization: Bearer <token>"`.
