...
In addition to the MCP SSE endpoint at `/sse`, the server exposes two utility endpoints:
...
**Authentication:** `ENGRAM_API_KEY` is required — the server refuses to start without it. `make init` generates a strong 32-byte hex token automatically. Every SSE connection must present `Authorization: Bearer <token>`. Connections without it are rejected with `401 Unauthorized`.
