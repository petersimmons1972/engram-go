...
| FM-16 | Hardcoded ClusterIP in NetworkPolicy | engram + aifleet netpols encoded a service's ClusterIP — service recreate silently broke egress with no alert | NetworkPolicy egress must use `podSelector`/`namespaceSelector`, never a hardcoded IP                     | 🤖    |
